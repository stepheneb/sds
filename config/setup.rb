...
This setup program will help you configure a new SDS instance.

HEREDOC

#
# check for config/environment.rb
#

environment_rb_path = rails_file_path(%w{config environment.rb})

unless File.exists?(environment_rb_path)
  puts <<HEREDOC
----------------------------------------

The Rails environment configuration file does not yet exist.

HEREDOC
  copy_file(rails_file_path(%w{config environment.sample.rb}), environment_rb_path)
end
...
HEREDOC
  ...
  db_config = YAML::load(IO.read(rails_file_path(%w{config database.sample.yml})))
  ...
  from template: #{RAILS_ROOT}/config/database.sample.yml
...
You will need to specify values for the SMTP mail server this SDS instance will
use to send outgoing mail. In addition you need to specify the hostname of this
specific SDS instance.

The SMTP parameters are used to send user account activation emails to new 
users and the hostname of the SDS is used as part of account activation url
rendered into the body of the email.

You will need to specify a mail delivery method: (#{deliv_types})

  the hostname of the SDS without the protocol: (example: #{mailer_config[:host]})
...
mailer_config[:host] =                     ask("    SDS hostname: ") { |q| q.default = mailer_config[:host] }
...
puts "\nTo complete the SDS Rails application setup run:\n\n  rake sds:setup:new_sds_from_scratch\n\n"
